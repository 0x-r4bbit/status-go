...
func (db *Database) GetCommunitiesSettings() ([]params.CommunitySettings, error) {
	rows, err := db.db.Query("SELECT community_id, message_archive_seeding_enabled, message_archive_fetching_enabled, last_message_archive_end_date FROM communities_settings")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&settings.CommunityID, &settings.MessageArchiveSeedingEnabled, &settings.MessageArchiveFetchingEnabled, &settings.LastMessageArchiveEndDate)
		if err != nil {
			return nil, err
		}
