...
func (m *Messenger) InitHistoryArchiveTasks(communities []*communities.Community) {
	...
	for _, c := range communities {
		...
		if c.Joined() {
			...
			if err != nil {
				log.Println("failed to get community settings", err)
			...
			if err != nil {
				log.Println("failed to get community chats filters", err)
				m.logger.Debug("failed to get community chats filters", zap.Error(err))
				continue
			}

			if len(filters) == 0 {
				log.Println("no filters or chats for this community starting interval", c.IDString())
				m.logger.Debug("no filters or chats for this community starting interval")
			...
			if err != nil {
				log.Println("failed to get Latest waku message timestamp", err)
			...
			if err != nil {
				log.Println("failed to request missing messages", err)
			...
			if err != nil {
				log.Println("failed to get archive partition start timestamp", err)
