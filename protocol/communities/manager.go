...
func (m *Manager) CreateHistoryArchiveTorrent(communityID types.HexBytes, topics []types.TopicType, startDate time.Time, endDate time.Time, partition time.Duration) error {
	...
	log.Println("Creating archives for start date: ", startDate, " till: ", endDate, " partitioned by ", partition)
	for {
		if from.Equal(endDate) || from.After(endDate) {
			break
		}
		log.Println("Creating message archive for partition (", partition, "). From: ", from, " To: ", to)
		...
		if len(messages) == 0 {
			...
			log.Println("No messages in this partition")
			from = to
			to = to.Add(partition)
			if to.After(endDate) {
				to = endDate
	...
	if len(encodedArchives) > 0 {
		...
		log.Println("No archives created")
		m.publish(&Subscription{
			NoHistoryArchivesCreatedSignal: &signal.NoHistoryArchivesCreatedSignal{
				CommunityID: communityID.String(),
				From:        int(startDate.Unix()),
